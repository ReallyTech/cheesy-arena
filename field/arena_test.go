...
func TestPlcMatchCycleGameSpecific(t *testing.T) {
	...
	assert.Equal(t, 0, redScore.FuelAuto)
	assert.Equal(t, 0, blueScore.FuelAuto)
	assert.Equal(t, [3]bool{false, false, false}, plc.redTowerLights)
	assert.Equal(t, [3]bool{false, false, false}, plc.blueTowerLights)
	plc.redFuelCount = 0
	plc.blueFuelCount = 0
	...
	plc.redFuelCount = 1
	arena.Update()
	assert.Equal(t, 1, redScore.FuelAuto)
	assert.Equal(t, 0, blueScore.FuelAuto)
	// Tower lights are off in auto/pause unless status is changed.
	assert.Equal(t, [3]bool{false, false, false}, plc.redTowerLights)
	...
	plc.redFuelCount = 2
	arena.Update()
	assert.Equal(t, 2, redScore.FuelAuto)
	assert.Equal(t, 0, blueScore.FuelAuto)
	...
	arena.Update()
	assert.Equal(t, 2, redScore.FuelAuto)
	assert.Equal(t, 1, blueScore.FuelAuto)

	redScore.TowerLevels[0] = 1
	arena.Update()
	assert.Equal(t, [3]bool{true, false, false}, plc.redTowerLights)

	blueScore.TowerLevels[1] = 2
	arena.Update()
	assert.Equal(t, [3]bool{false, true, false}, plc.blueTowerLights)

	// Check the tower lights during the "sonar ping" warning sound.
	...
	assert.Equal(t, TeleopPeriod, arena.MatchState)
	assert.Equal(t, [3]bool{true, false, false}, plc.redTowerLights)
	assert.Equal(t, [3]bool{true, false, false}, plc.blueTowerLights)
	arena.MatchStartTime = time.Now().Add(-durationToWarning - 300*time.Millisecond)
	arena.Update()
	assert.Equal(t, TeleopPeriod, arena.MatchState)
	assert.Equal(t, [3]bool{false, true, false}, plc.redTowerLights)
	assert.Equal(t, [3]bool{false, true, false}, plc.blueTowerLights)
}
