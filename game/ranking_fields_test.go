...
func TestAddScoreSummary(t *testing.T) {
	...
	assert.Equal(t, RankingFields{5, 40, 30, 10, 15, 0.9451961492941164, 1, 0, 0, 0, 1}, rankingFields)

	// Add a win.
	rankingFields.AddScoreSummary(blueSummary, redSummary, false)
	assert.Equal(t, RankingFields{6, 60, 44, 15, 15, 0.24496508529377975, 1, 1, 0, 0, 2}, rankingFields)

	// Add a tie.
	rankingFields.AddScoreSummary(redSummary, redSummary, false)
	assert.Equal(t, RankingFields{9, 100, 74, 25, 30, 0.6559562651954052, 1, 1, 1, 0, 3}, rankingFields)

	// Add a disqualification.
	rankingFields.AddScoreSummary(blueSummary, redSummary, true)
	assert.Equal(t, RankingFields{9, 100, 74, 25, 30, 0.05434383959970039, 1, 1, 1, 1, 4}, rankingFields)
}

func TestSortRankings(t *testing.T) {
	// Check tiebreakers (RP, Fuel, Tower, AutoFuel, AutoTower, Random).
